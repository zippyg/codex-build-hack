...
The public schema base is illustrative until the name/domain is cleared. Checked-in schemas use stable `https://promptectomy.invalid/schemas/v2/` identifiers so no unapproved live domain is implied.

The canonical implementation is `engine/schemas/v2/contract.schema.json`. Generated Python, TypeScript, and Rust transport bindings embed that file's SHA-256 digest and are reproducibly regenerated by `engine/scripts/generate_contract_bindings.py`. JSON Schema remains authoritative. Client bindings do not weaken runtime validation.
