...
_SAFE_OPERATIONS = {"chat", "execute_tool", "responses", "responses.create", "responses.parse"}
...
def _metadata_shape(value: Any, *, depth: int = 0) -> Any:
    if depth > 8:
        raise EvidenceError("capture_shape_limit_exceeded", "Capture shape exceeds the depth bound")
    if value is None:
        return "null"
    ...
    if isinstance(value, (list, tuple)):
        if len(value) > 256:
            raise EvidenceError("capture_shape_limit_exceeded", "Capture array exceeds the item bound")
        return {"array": [_metadata_shape(item, depth=depth + 1) for item in value[:16]], "count": len(value)}
    if isinstance(value, dict):
        if len(value) > 256 or any(not isinstance(key, str) for key in value):
            raise EvidenceError("capture_shape_limit_exceeded", "Capture object exceeds the member bound")
        return {
            "object": sorted(
    ...
    raise EvidenceError("capture_shape_unsupported", "Capture shape contains an unsupported runtime value")
...
def capture_metadata(
    ...
    *,
    ...
    if not record.attempt_id or len(record.attempt_id) > 256 or not record.callsite_id:
        raise EvidenceError("malformed_capture", "Capture identity is missing or oversized")
    ...
    if not set(record.flags) <= _CAPTURE_FLAGS:
        raise EvidenceError("malformed_capture", "Capture contains an unsupported semantic flag")
    if len(record.provider) > 128 or len(record.model) > 256:
        raise EvidenceError("malformed_capture", "Capture provider or model is oversized")
    request_shape = _metadata_shape(record.request_shape)
    response_shape = _metadata_shape(record.response_shape)
    correlation = _correlate({"promptectomy.callsite_id": record.callsite_id}, callsites)
    return NormalizedObservation(
        observation_id=_digest("capture", record.runtime, record.attempt_id, str(start)),
        ...
        input_tokens=None if record.input_tokens is None else _bounded_int(record.input_tokens, field="input_tokens"),
        output_tokens=None if record.output_tokens is None else _bounded_int(record.output_tokens, field="output_tokens"),
...
def _bounded_int(value: Any, *, field: str, minimum: int = 0) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
        raise EvidenceError("malformed_span", f"{field} must be an integer at least {minimum}")
    if value > 2**64 - 1:
        raise EvidenceError("malformed_span", f"{field} exceeds the OTLP unsigned integer range")
    return value


def _any_value(value: Any, limits: ImportLimits) -> str | int | bool | None:
    if not isinstance(value, dict) or len(value) != 1:
        raise EvidenceError("malformed_span", "OTLP attribute value must contain one typed value")
    kind, item = next(iter(value.items()))
    if kind == "stringValue":
        if not isinstance(item, str) or len(item.encode()) > limits.max_attribute_value_bytes:
            raise EvidenceError("attribute_limit_exceeded", "OTLP string attribute exceeds its bound")
        return item
    if kind == "intValue":
        if isinstance(item, str):
            try:
                ...
                raise EvidenceError("malformed_span", "OTLP integer attribute is invalid") from exc
    ...
    if kind in {"arrayValue", "kvlistValue", "bytesValue", "doubleValue"}:
        return None
    raise EvidenceError("malformed_span", "OTLP attribute uses an unknown value encoding")


def _attributes(items: Any, limits: ImportLimits) -> tuple[dict[str, str | int | bool | None], int, tuple[str, ...]]:
    if items is None:
        return {}, 0, ()
    if not isinstance(items, list) or len(items) > limits.max_attributes_per_span:
        raise EvidenceError("attribute_limit_exceeded", "OTLP span attribute count exceeds its bound")
    mapped: dict[str, str | int | bool | None] = {}
    unknown = 0
    warnings: set[str] = set()
    for item in items:
        if not isinstance(item, dict) or set(item) != {"key", "value"}:
            raise EvidenceError("malformed_span", "OTLP attribute must contain only key and value")
        key = item["key"]
        if not isinstance(key, str) or not key or len(key.encode()) > limits.max_attribute_key_bytes:
            raise EvidenceError("attribute_limit_exceeded", "OTLP attribute key is invalid or oversized")
        ...
        if key not in _MAPPED_ATTRIBUTES:
            ...
            continue
        if key in mapped:
            raise EvidenceError("malformed_span", "OTLP span contains duplicate mapped attributes")
...
def _correlate(attributes: dict[str, Any], callsites: tuple[CallsiteCandidate, ...]) -> Correlation:
    ...
    path_matches = [item for item in callsites if isinstance(path, str) and item.relative_path == path]
    symbol_matches = [item for item in callsites if isinstance(symbol, str) and item.symbol == symbol]
...
def _status(span: dict[str, Any], attributes: dict[str, Any]) -> Literal["ok", "error", "cancelled", "unknown"]:
...
def _shape_digest(attributes: dict[str, Any], prefix: str) -> str:
    keys = sorted(key for key in attributes if key.startswith(prefix) and key in _MAPPED_ATTRIBUTES)
...
def _normalize_span(
    ...
    if any(field not in span for field in required):
        raise EvidenceError("malformed_span", "OTLP span lacks an identity or timestamp")
    trace_id = span["traceId"]
    span_id = span["spanId"]
    if not isinstance(trace_id, str) or not isinstance(span_id, str) or not trace_id or not span_id:
        raise EvidenceError("malformed_span", "OTLP trace and span identities must be non-empty strings")
    ...
    raw_model = _text(attributes, "gen_ai.response.model", _text(attributes, "gen_ai.request.model", "unknown"))
...
def _spans(document: Any) -> list[Any]:
    if not isinstance(document, dict) or set(document) - {"resourceSpans", "partialSuccess"}:
        raise EvidenceError("malformed_otlp", "OTLP JSON root is unsupported")
    resources = document.get("resourceSpans")
    if not isinstance(resources, list):
        raise EvidenceError("malformed_otlp", "OTLP JSON resourceSpans must be an array")
    spans: list[Any] = []
    for resource in resources:
        if not isinstance(resource, dict):
            raise EvidenceError("malformed_otlp", "OTLP resource span must be an object")
        ...
        for scope in scopes:
            if not isinstance(scope, dict) or not isinstance(scope.get("spans", []), list):
                raise EvidenceError("malformed_otlp", "OTLP scope span must contain a spans array")
...
def import_otlp_http_json(
    ...
    *,
    ...
    if len(payload) > limits.max_payload_bytes:
        raise EvidenceError("payload_limit_exceeded", "OTLP payload exceeds its byte bound")
    try:
        document = parse_json_strict(payload)
    except ContractError as exc:
        raise EvidenceError("malformed_otlp", "OTLP payload is not strict JSON") from exc
    spans = _spans(document)
    if len(spans) > limits.max_spans:
        raise EvidenceError("backpressure_limit_exceeded", "OTLP span count exceeds the admission bound")
    ...
    for span in spans:
        try:
            observation, span_warnings, identity = _normalize_span(span, callsites, limits)
    ...
    if spans and not accepted and rejected_count:
        raise EvidenceError("all_spans_rejected", "No OTLP span passed the bounded mapping contract")
    accepted.sort(key=lambda item: (item.observed_at_unix_nano, item.observation_id))
    partial_success = isinstance(document, dict) and bool(document.get("partialSuccess"))
...
def import_otlp_grpc(
    ...
    *,
    ...
    try:
        ...
        raise EvidenceError("malformed_otlp", "OTLP gRPC payload is not an ExportTraceServiceRequest") from exc
    known_fields = ExportTraceServiceRequest()
    known_fields.CopyFrom(request)
    known_fields.DiscardUnknownFields()
    if known_fields.SerializeToString(deterministic=True) != request.SerializeToString(deterministic=True):
        raise EvidenceError("unknown_otlp_fields", "OTLP gRPC payload contains unsupported unknown fields")
