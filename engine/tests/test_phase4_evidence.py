...
def test_duplicate_out_of_order_partial_and_malformed_spans_are_explicit() -> None:
    ...
    report = import_otlp_http_json(_payload([late, early, early, malformed], partial=True))
    assert [item.observed_at_unix_nano for item in report.observations] == [1_000, 5_000]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_payload([_span(), _span(span_id="c" * 16)]), ImportLimits(max_spans=1), "backpressure_limit_exceeded"),
        (_payload([]), ImportLimits(max_payload_bytes=1), "payload_limit_exceeded"),
        (_payload([{"traceId": "x"}]), ImportLimits(), "all_spans_rejected"),
    ],
)
def test_import_limits_and_strict_json_fail_typed(payload: bytes, limits: ImportLimits, code: str) -> None:
...
def test_correlation_never_silently_upgrades_ambiguous_or_weak_evidence() -> None:
    ...
    path_and_symbol = _span(
        ...
        attributes=[_attribute("code.filepath", "src/a.py"), _attribute("code.function.name", "send")],
    )
    path_only = _span(span_id="2" * 16, start=2_000, end=3_000, attributes=[_attribute("code.filepath", "src/b.py")])
    ambiguous = _span(span_id="3" * 16, start=3_000, end=4_000, attributes=[_attribute("code.function.name", "send")])
    unmatched = _span(span_id="4" * 16, start=4_000, end=5_000, attributes=[_attribute("code.function.name", "other")])
    report = import_otlp_http_json(_payload([path_and_symbol, path_only, ambiguous, unmatched]), callsites=callsites)
    correlations = [item.correlation for item in report.observations]
    assert [(item.callsite_id, item.confidence, item.reason) for item in correlations] == [
...
def test_otlp_grpc_protobuf_uses_the_same_bounded_normalization() -> None:
    ...
    ParseDict(json.loads(_payload([_span(attributes=[_attribute("gen_ai.system", "openai")])])), request)
...
def test_runtime_capture_is_metadata_only_across_semantic_cells(runtime: str) -> None:
    ...
    assert observation.callsite_id == callsite_id
    assert observation.flags == ("cancellation", "retry", "streaming", "structured_output", "tools")
...
def test_runtime_capture_rejects_unbounded_or_unknown_shapes() -> None:
    ...
    with pytest.raises(EvidenceError) as caught:
        capture_metadata(CaptureRecord(**base, request_shape={"items": list(range(257))}))
